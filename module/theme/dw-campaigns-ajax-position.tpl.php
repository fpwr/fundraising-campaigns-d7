<?php
    ...
    // we get "pcp" and "possible" - need thisUser, campaign, pcp, possible
    $userId       = _dw_campaigns_contact_id_get_user($pcp->contact_id);
    ...
    $goalRemaining  = dw_campaigns_force_decimal($goalRemaining, $campaign->field_dw_currency['und']['0']['value']);    
?>
...
<div class="how-we-will-do">
    ...
    <!-- <span class="words"><?php echo $thisUser->data['displayname']; ?> will sit in</span> -->
    ...
    <div class="updated-leaderboard">
        ...
        <?php
            ...
            foreach($pos as $contact_id => $total) {
                ...
                $displayname = isset($fake_user->data['displayname']) ? $fake_user->data['displayname'] : '(unavailable)';
        ?>
                <tr class="<?php echo $class; ?>">
                    <td class="name">
                        <img src="<?php echo $photo; ?>" width="25"> <a href="<?php echo $url; ?>"> <?php echo $displayname;?></a>
                    </td>
                    <?php
                    ...
                    <?php
                        ...
                        if($diff < 0) {
                            $diff_sentence = dw_campaigns_force_decimal(abs($diff), $campaign->field_dw_currency['und']['0']['value']) . " behind";
                        } else {
                            $diff_sentence = dw_campaigns_force_decimal(abs($diff), $campaign->field_dw_currency['und']['0']['value']) . " ahead";
                        }
                    ?>
                    <td class="leader-diff">
                    	<div class="leader-total"><?php echo dw_campaigns_force_decimal($total, $campaign->field_dw_currency['und']['0']['value']); ?></div>
