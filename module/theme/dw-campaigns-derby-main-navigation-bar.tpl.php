<?php
    ...
    $last_search   = dw_campaigns_get_last_search();
    $default_text  = empty($last_search) ? 'search for a fundraiser' : $last_search;
?>
<div class="nav-bar">
    <ul>
        ...
        <li><a href="<?php echo $derby_url; ?>" target="_blank"><?php echo t('About the Derby'); ?></a></li>
        <li class="<?php echo $leaderboard_class; ?>"><a href="<?php echo $leaderboard_url; ?>"><?php echo t('Leader Board'); ?></a></li>
        <?php if(!empty($give_url)) {
        ?>
        <li><a href="<?php echo $give_url; ?>" target="_blank">Give</a></li>
        <?php
        }
        ?>
        <?php
        ...
        <li class="search"><form method="post" action="<?php echo $search_url; ?>"><input type="text" name="query" class="nav-search form-text <?php if(empty($last_search)) { echo 'has-default-text';}?>" value="<?php echo $default_text;?>"><input type="submit" value="<?php echo t('Go'); ?>" class="form-submit"></form></li>
    </ul>
</div>
